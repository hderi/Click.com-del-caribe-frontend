...
export default function ClienteDetallePage({ params }) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/clientes/${client.id}`, {
        method: "PATCH",
        headers,
      ...
      if (res.ok) {
        ...
        setClients(prev => prev.map(c => c.id === client.id ? normalizeClient(updated.cliente) : c));
        setIsEditing(false);
      } else {
    ...
    return (
      <div className="rounded-3xl border border-[#C9D8E5] bg-white p-8">
        <h1 className="text-2xl font-black text-[#102033]">Cliente no encontrado</h1>
          <button type="button" className="rounded-xl border border-[#B8CFE0] bg-white px-4 py-2 text-sm font-semibold text-[#12304A] shadow-sm hover:bg-[#EAF5FB]">Editar datos</button>
    ...
    <div className="space-y-6">
        ...
        {isEditing ? (
          <form onSubmit={handleSave} className="mt-4 space-y-4">
            <div>
              ...
              <input type="text" value={editForm.nombre || ""} onChange={e => setEditForm({...editForm, nombre: e.target.value})} className="w-full rounded-xl border border-[#C9D8E5] p-3 text-[#102033]" required />
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="block text-sm font-black text-[#102033]">Teléfono</label>
                <input type="text" value={editForm.telefono || ""} onChange={e => setEditForm({...editForm, telefono: e.target.value})} className="w-full rounded-xl border border-[#C9D8E5] p-3 text-[#102033]" required />
              </div>
              <div>
                <label className="block text-sm font-black text-[#102033]">Correo</label>
                <input type="email" value={editForm.correo || ""} onChange={e => setEditForm({...editForm, correo: e.target.value})} className="w-full rounded-xl border border-[#C9D8E5] p-3 text-[#102033]" />
              </div>
            </div>
      ...
      <section id="historial" className="rounded-[28px] border border-[#C9D8E5] bg-white p-6 shadow-[0_14px_34px_rgba(15,23,42,0.08)]">
        ...
        {clientRepairs.length === 0 ? (
          <p className="mt-3 text-sm text-[#526174]">Este cliente aún no tiene reparaciones registradas.</p>
        ) : (
          <div className="mt-4 space-y-3">
            {clientRepairs.map((repair) => (
              <Link key={repair.folio} href={`/admin/reparaciones/${repair.folio}`} className="flex items-center justify-between rounded-2xl border border-[#C9D8E5] bg-[#F8FBFD] p-4 hover:bg-[#E8F1F8]">
                <div>
